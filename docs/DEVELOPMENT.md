...
> **Target:** Aerogel `26.2-1`, Minecraft Java Edition `26.2+`, Java `25`  
...
Until the Gradle plugin is published to a public plugin repository, extract `aerogel-gradle-plugin-26.2-1.zip` and point `pluginManagement` at the extracted Maven repository:

```kotlin
// settings.gradle.kts
pluginManagement {
    repositories {
        maven { url = uri("path/to/extracted/aerogel-gradle-plugin-26.2-1") }
        mavenCentral()
        gradlePluginPortal()
    }
}
