...
- Current baseline: Aerogel `26.2-5`, Minecraft server `26.2`, Java `25`, Kotlin `2.4.10`.
- Aerogel release versions follow `<minecraft-version>-<revision>`, for example `26.2-5`.
...
pluginManagement {
    repositories {
        maven { url = uri("C:/path/to/aerogel-gradle-plugin-26.2-5") }
        mavenCentral()
        gradlePluginPortal()
    }
}
