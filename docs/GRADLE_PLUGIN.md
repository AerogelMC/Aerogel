...
Then apply `id("dev.aerogel.plugin") version "26.2-3"` normally. The `maven` branch is
updated automatically after changes reach `main` and retains previously published versions.

## Local repository package

For offline development or a pinned local mirror, extract `aerogel-gradle-plugin-26.2-3.zip` and add that directory to `settings.gradle.kts`:
